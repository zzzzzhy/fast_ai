...
dtype_dict = {}
print("Compiling the model...", data.shape)
with nnvm.compiler.build_config(opt_level=2):
    graph, lib, params = nnvm.compiler.build(sym, target, shape, dtype_dict, params, target_host='llvm -mfloat-abi=soft -target=armv7a-a-linux-android')
...
def save_lib():
    ...
    lib.export_library('3288/yolov2.tar')
    with open("3288/yolov2", "w") as fo:
        fo.write(graph.json())
    with open("3288/yolov2.params", "wb") as fo:
...
boxes = []
region_layer = net.layers[net.n - 1]
boxes, probs = nnvm.testing.yolo_detection.get_region_boxes(region_layer, im_w, im_h, net.w, net.h,
                       thresh, probs, boxes, 1, tvm_out)

boxes, probs = nnvm.testing.yolo_detection.do_nms_sort(boxes, probs,
