...
with nnvm.compiler.build_config(opt_level=2):
    graph, lib, params = nnvm.compiler.build(sym, target, shape, dtype_dict, params, target_host='llvm -target=arm64-linux-android -mattr=+neon')
...
def save_lib():
    ...
    lib.export_library('yolo.tar')
    with open("yolo", "w") as fo:
        fo.write(graph.json())
    with open("yolo.params", "wb") as fo:
