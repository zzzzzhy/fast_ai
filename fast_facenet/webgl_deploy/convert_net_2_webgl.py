...
def convert(net, params, data_shape, dtype,target, target_host):
    # compile
    ...
    with open("./net2", "w") as fo:
        fo.write(graph.json())
    with open("./net2.params", "wb") as fo:
...
convert(nnvm_net, nnvm_params, (1, 3, 112, 112) ,'float32','opengl', 'llvm -target=arm64-linux-android')
